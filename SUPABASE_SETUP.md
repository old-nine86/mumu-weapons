...
3. 如果要用 AI 精修，在顶部填 AI API Key 并保存。这个 key 只保存在当前浏览器会话，关闭页面后清空，不写进 GitHub。
...
- 现在前端默认每个访客免费上传 50 件，方便前期内测和推广收集作品。
- 长期目标可以改成每个 IP/账号免费上传 2 件。
...
重要：严格按 IP/账号限制不能只靠 GitHub Pages 前端完成。需要下一步加 Supabase Edge Function 或独立后端，因为只有服务端函数才能可靠读取请求 IP、计算剩余额度、审核推广奖励并防止绕过。

## 现在的限制

- 当前网页端仍保留浏览器本地限制，用来防止普通误传，但不能当作强防刷系统。
- 论坛现在是公开发布、公开读取；名字可以自填，不等于真实账号身份。如果以后要做身份防冒用或“论坛也必须先审核再展示”，需要加账号登录和论坛审核台。
- 严格“每 IP/账号免费 2 件、推广后 +10 件”需要再加 Supabase Edge Function 或自己的后端接口。
